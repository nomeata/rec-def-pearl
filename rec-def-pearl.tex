...
\begin{code}
rTrans1 :: Graph -> Graph
rTrans1 g = M.map S.toList reaches where
...
\begin{code}
rTrans2 :: Graph -> Graph
rTrans2 g = M.map (S.toList . RS.get) reaches where
...
\end{itemize}
\Citet{clairvoyant} give a denotational semantics with a cost model of lazy evaluation, and in their semantic domain knot-tying can be recognized. We are currently trying to figure out if we can solve the second problem in that setting as well.
