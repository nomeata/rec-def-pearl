...
\newcommand{\heart}{\textcolor{red}{\ensuremath\varheartsuit}}
...
\begin{frame}[fragile]{I {\heart} Laziness}
\begin{center}
\textbf{Laziness allows recursive definitions!}
\end{center}
{\pause}
...
\begin{frame}[fragile]{C is lazy, Haskell is lazier}
\begin{center}
\begin{tabular}{c|c|c}
...
\begin{frame}[fragile]{A famous example}
...
\path<3>[red,draw,->] (1) to [bend right] (2);
\end{tikzpicture}
\lstinline-) = -
\Alt<3>{$\bot$}{%
\begin{tikzpicture}[anchor=base,baseline]
...
M.map S.toList reaches
  where
    reaches :: M.Map Int (S.Set Int)
    reaches = M.fromList [(1, s1),(2,f s2),(3, f s3)]
    f v vs = S.insert v (S.unions [ reaches M.! v' | v' <- vs ]))
    s1 = f 1 [2,3]
    s2 = f 2 [1]
    s3 = f 3 []
...
\begin{frame}[fragile,t]{Let’s step through it}
\begin{lstlisting}
...
M.fromList [(1, S.toList s1),(2, S.toList s2),(3, S.toList s3)]
  where



    @1s1@ = S.insert 1 (@2s2@ `S.union` s3)
    @3s2@ = S.insert 2 @4s1@
    s3 = S.fromList [3]
\end{lstlisting}%
\only<2>{%
...
\section{This does not work {\ldots} could it?}
...
\iffalse
\begin{frame}[fragile,t]{Almost the same program}
\begin{lstlisting}
rtrans :: Graph -> Graph
rtrans g = M.map S.toList reaches
  where
...
\section{It worked!\\\small(And there are more examples, but not today\ldots)}

\begin{frame}
\centering
\Large
\url{https://hackage.haskell.org/package/rec-def}
\end{frame}

\begin{frame}[fragile]{Solves every set of equations!}
...
% We could have
% \begin{lstlisting}
% RS.map          :: Ord b => (a -> b) -> RSet a -> RSet b
% \end{lstlisting}
% but ther are other issues.
\end{frame}
...
\begin{frame}{Summary}
\begin{itemize}
\item Laziness is key to describing recursive problems declaratively.
\item Let us allow more partial orders than Haskell’s “normal one”!
\item Open question: Is this still pure, and how to prove it?
\item Not discussed today:\\
The \lstinline-let x = x- problem, thread safety, avoiding leaks, performance.
\end{itemize}
...
\section{It worked!\\Let's try another example\ldots}
...
\begin{frame}[fragile,t]{Data.Recursive.Bool to the rescue!}
\begin{lstlisting}
ghci> someVal = Lam "y" (Var "y")
ghci> prog = LetRec [("x", App (Var "x") someVal), ("y", Throw)] (Var "x")
ghci> canThrow1 prog
...
\begin{frame}[fragile,t]{JFR: The full code}
