...
He explained that the use of |unlift| on the expression to be analyzed is unsafe, but ok here, because the expression is invariant here; correctly tracking the monotonicity of user-defined data types like |Exp| is possible, but requires auxiliary functions.
...
The datafix library mentioned in \cref{sec:datafix} was created in particular due to the desire to free the equations to be solved from the syntactic structure of the input program. Graf shows how the case study can be implemented using datafix, and went straight for the harder problem of a combined analysis/transformation pass, corresponding to \cref{sec:proganalexp}.
