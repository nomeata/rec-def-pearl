...
A maybe more practical example is the following calculation of the reflexive transitive closure of a graph, i.e.\ for each node the set of nodes reachable from it. Let's represent a graph as a map from vertices (|Int|) to lists of adjacent vertices:
\begin{code}
...
\end{code}
This code is quite close to the prosaic specification “the reachable nodes from a node |v| are |v| itself, plus all the nodes reachable from its successors”; hence we can call this code declarative.

Note how the definition of |sets| refers to itself -- “we are tying a knot”.
...
But it seems we should: The declarative specification, from which we have derived |transitive1|, holds for recursive graphs as well, so it does not seem that unreasonable to expect our code to handle recursive graphs as well. Where does it go wrong? The way we use the lazy map data structure is fine; it helps us to express the set of reachable nodes by way of other such sets.
The problem is that the set data structure, with its operations |insert| and |union|, is not lazy enough: |union| needs to know the value of its arguments before it can produce something useful, and thus cannot be used recursively.

\subsection{We need better sets!}

In this paper we present a data structure for sets, called |RSet|, where such recursive expressions do work! Its API is almost the same as that of |Data.Set|. In particular, it consists of plain \emph{pure} functions -- no monads necessary. The fragment relevant for our example is:
\begin{code}
insert  :: Ord a =>  a -> RSet a  -> RSet a
unions  :: Ord a =>  [RSet a]     -> RSet a
get     ::           RSet a       -> Set a
\end{code}
In addition to the two operations used by |transitive1|, with type signatures mirroring those of |Data.Set| exactly, we find the function |get| to convert our |RSet| to a normal |Set|.

We can use this data structure without changing the structure of our code; we just swap out the operations (imported qualifiedly as |RS|) and convert back to conventional sets in the end:
\begin{code}
...
\item that the definition of a |x| does not depend on |RS.get x|
\end{itemize}
...
These data types -- |RSet|, |RBool|, |RDualBool| are not silos, and you will find among the functions in \cref{fig:api} some that connect these types -- negation on Booleans, member checks on sets, the emptiness check on sets, and the function |when|, which guards the values of a set on a boolean.
...
Alas, this does not work: As soon as we try to analyze an expression that uses recursion, we will fall into a black hole. The crux is that |Bool| is not recursively definable, because the operations (here only disjunction, |(||||)|) is not lazy.
...
At this point, the actual implementation of this API is not that interesting. In a simple implementation a cell would consist of a current value (initialized to the empty set), and a list of cells depending on this value, and then the changes due to calls to |cellIsInsert| are propagated through this network until no more changes need to be propagated. Of course, more sophisticated algorithms may lurk underneath this interface.
...
Furthermore, it has to be lazy in its second argument, else a recursive equation would immediately loop, so it somehow has to defer the call to |cellIsInsert| a bit. This leads to the code seen in \cref{fig:wrap}, which we go through in detail:
...
\item An |IO ()| action, deferred until the the value is actually needed (directly due to |get|, or indirectly).
...
Certainly purity ought to imply memory and type safety: What we get out of |RS.get| should really be a valid |Set|, and not some corrupted memory.
...
As you ponder this question, note that if you include (asymptotic) resource usage in your expectation of program equivalence, then such lambda-lifting is already rather dangerous in Haskell: The nice and fast knot-tied |fibs| from the introduction becomes horribly inefficient once you turn it into a recursive \emph{function} (|let fibs x y = x : y  : ...|). Because the lamda-lifting transformation is only a equivalence as long as we ignore (asymptotic) complexity, it seems that breaking that equivalence is not too wicked.

\Citet[Fact 3.7]{sabry} points out that merely changing the set of observational equivalences does, in general, \emph{not} imply that the extended language is not pure.
...
The maybe most similar work is \citeauthor{runST}'s \citeyear{runST} proof that |runST :: ST -> a| is safe, but it does not yet cover laziness nor concurrency.
...
In the introduction we mentioned that without the data structure presented in this paper, a programmer likely has to write their reflexive-transitive-closure code with an explicit loop (a tail-recursive |go| function), explicitly keeping track of seen nodes to avoid running in circles:
\begin{code}
transitive3 :: Graph -> Graph
transitive3 g = M.fromList [ (v, S.toList (go S.empty [v])) | v <- M.keys g ]
  where
