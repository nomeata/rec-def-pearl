...
%Datafun comes with a nice denotational semantics, which we lack so far. The main hurdle we are facing is that mutually recursive |let| builds fixed-points both in Haskell's 
...
Recursive equations still have to be manually turned into a single equation and threaded through an the explicit |fix| operator provided by Hatafun; here we believe our approach of using Haskell's |let| to declare complex mutually recursive equations provides significant benefits (as discussed in \cref{sec:casestudyalts}.
...
The same algorithm as in \cref{sec:proganalexp} can be implemented using the datafix library described in \cref{sec:datafix} (code provided by Sebastian Graf):
